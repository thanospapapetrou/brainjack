...
public class Iteration implements Command {
  private static final String INVALID_END_TOKEN =
      String.format("End token must be %1$s", TokenType.RIGHT_SQUARE_BRACKET);
  private static final String INVALID_START_TOKEN =
      String.format("Start token must be %1$s", TokenType.LEFT_SQUARE_BRACKET);
  ...
  public Iteration(final Token startToken, final Token endToken, final List<Command> commands) {
    requireValidToken(startToken, NULL_START_TOKEN, TokenType.LEFT_SQUARE_BRACKET,
        INVALID_START_TOKEN);
    requireValidToken(endToken, NULL_END_TOKEN, TokenType.RIGHT_SQUARE_BRACKET, INVALID_END_TOKEN);
